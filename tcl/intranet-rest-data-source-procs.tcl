...
ad_proc im_rest_project_task_tree_action {
	...
	default {
	    ...
	    doc_return 400 "application/json" "{\"success\": false, \"message\": \"tree_action: found invalid action=[im_quotejson $action]\"}"
	    return
	}
    }
    # The calling procedure will return a suitable JSON message
}
...
ad_proc im_rest_project_task_tree_update {
	...
	doc_return 400 "application/json" "{\"success\": false, \"message\": \"Did not find project_id in JSON data: [im_quotejson $var_hash_list]\"}"
	return
    }
	...
	doc_return 401 "application/json" "{\"success\": false, \"message\": \"User #$current_user_id ([im_name_from_user_id $current_user_id]) has not enough permissions to modify $object_type #$project_id ([acs_object_name $project_id])\"}"
	return
    }
...
ad_proc im_rest_project_task_tree_delete {
	...
	doc_return 400 "application/json" "{\"success\": false, \"message\": \"Delete failed because we did not find project_id in JSON data: [im_quotejson $var_hash_list]\"}"
	return
    }
	...
	doc_return 401 "application/json" "{\"success\": false, \"message\": \"No permissions to admin project_id=$project_id for user=$current_user_id\"}"
	return
    }
	...
	if {"" ne $err_msg} {
	    ...
	    doc_return 500 "application/json" "{\"success\": false, \"message\": \"[im_quotejson $err_msg]\"}"
	    return
	}
    }
}
...
ad_proc im_rest_project_task_tree_create {
	...
	doc_return 409 "application/json" "{\"success\": false, \"message\": \"Create failed, project_id=$project_id already exists. JSON data: [im_quotejson $var_hash_list]\"}"
	return
    }
	...
	doc_return 400 "application/json" "{\"success\": false, \"message\": \"Create failed, no parent_id specified for new task in post data: [im_quotejson $var_hash_list]\"}"
	return
    }
	...
	doc_return 401 "application/json" "{\"success\": false, \"message\": \"No permissions to write to parent_id=$parent_id for user=$current_user_id\"}"
	return
    }
